...
- [x] `client.ts` 작성
- [x] supported model family validation helper 작성
- [x] Bedrock body decode helper 작성

### Step 3 — Embedding 구현

- [x] `BedrockEmbeddingOptions` 타입 정의
- [x] `bedrockEmbedding()` 팩토리 함수 작성
- [x] Titan v2 request body 분기 구현
- [x] `embedMany()` serial loop 구현
- [x] 에러 메시지 정리

### Step 4 — Generation 구현

- [x] `BedrockGenerationOptions` 타입 정의
- [x] `bedrockGeneration()` 팩토리 함수 작성
- [x] `generate()` with `ConverseCommand`
- [x] `generateStream()` with `ConverseStreamCommand`
- [x] system prompt / history / inferenceConfig 반영

### Step 5 — 테스트 및 검증

- [x] `embedding.test.ts` 작성
- [x] `generation.test.ts` 작성
- [x] `pnpm turbo run build --filter=@ragpipe/plugin-bedrock`
- [x] `pnpm turbo run typecheck --filter=@ragpipe/plugin-bedrock`
- [x] `pnpm turbo run test --filter=@ragpipe/plugin-bedrock`
